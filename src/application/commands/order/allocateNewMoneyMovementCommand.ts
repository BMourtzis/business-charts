...
export interface AllocateNewMoneyMovementCommand {
    orderId: string;
...
export class AllocateNewMoneyMovementCommandHandler {
    constructor(
        private _orderStore = useOrdersStore(),
        private _moneyMovementStore = useMoneyMovementStore()
    ) {}

    async handler(cmd: AllocateNewMoneyMovementCommand) {
        const order = await this.getOrder(cmd.orderId);

        const movement = await this.createMoneyMovement(
            order.partnerId,
            order.type,
            cmd.movementAmount,
            cmd.method
        ...
        this.allocateMoneyToOrder(order, movement.direction, movement.id, cmd.allocationAmount);

        this.saveMoneyMovement(movement);
